...
train_boost_no_tuned <- train(SHOT_RESULT ~ ., data = train_set_RF,
                   method = "gbm")
##this took forever and still only got to .6191 used a tuned xgboost instead...need to convert to 0,1 even for gbm
########
fit_boost_no_tuned <- gbm(SHOT_RESULT ~ ., data = train_set_RF,
                          n.trees = 100,
                          interaction.depth = 3, 
                          shrinkage = 0.1,
                          n.minobsinnode = 10)
y_hat_rf_boost <- predict(fit_rf_ranger, test_set_RF)
```
