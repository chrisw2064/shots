...
#JUST DROP sho dist that equal 0
...
#u <- shots %>% group_by(CLOSEST_DEFENDER) %>% nest() %>% select(CLOSEST_DEFENDER)
#test <- shots %>% group_by(CLOSEST_DEFENDER, CLOSEST_DEFENDER_PLAYER_ID) %>% nest() %>% select(CLOSEST_DEFENDER, CLOSEST_DEFENDER_PLAYER_ID) %>% group_by(CLOSEST_DEFENDER) %>% count(CLOSEST_DEFENDER_PLAYER_ID) %>% arrange(desc(`n`))
#test[test$n > 1, ]
#length(test$CLOSEST_DEFENDER_PLAYER_ID)
