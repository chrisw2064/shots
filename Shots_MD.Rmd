...
shots <- shots %>% mutate_at("SHOT_RESULT", as.factor) %>% mutate(SHOT_NUMERIC = as.numeric(SHOT_RESULT == "made"))
...
#now fit a model
fit_glm_shots <- glm(SHOT_NUMERIC ~ qtr_seconds + SHOT_CLOCK + DRIBBLES + TOUCH_TIME + PTS_TYPE + CLOSE_DEF_DIST*SHOT_DIST*MISMATCH, data = train_set_glm_shots, family = "binomial")
...
variables_to_use <- shots %>% select(qtr_seconds, SHOT_CLOCK, DRIBBLES, TOUCH_TIME, PTS_TYPE, CLOSE_DEF_DIST, SHOT_DIST, MISMATCH) %>% names(.)
...
fit_rf_ranger <- ranger(SHOT_RESULT ~ ., data = train_set_RF, 
                  ...
                  mtry = train_rf_n0_tuned$bestTune$mtry,
...
########
library(xgboost)
#this will not be needed if slice is not maked by xgboost package
...
train_set_RF_noslice <- train_set_RF_noslice %>% mutate(SHOT_RESULT = as.numeric(SHOT_RESULT == "made"))
test_set_RF_noslice <- test_set_RF_noslice %>% mutate(SHOT_RESULT = as.numeric(SHOT_RESULT == "made"))
#
...
xgbcv <- xgb.cv( params = params_default_xgboost, data = matrix_train_xgboost, label = xg_labels, nrounds = 100, nfold = 5, showsd = T, stratified = T, print_every_n = 10, early_stop_round = 20, maximize = F)
xgbcv$evaluation_log$iter[xgbcv$evaluation_log$test_error_mean == min(xgbcv$evaluation_log$test_error_mean)]
#
xgb_grid_1 <- expand.grid(eta = 0.1, 
...
xgb_train_1
xgb_predictions <- predict(xgb_train_1, test_set_RF_noslice)
confusionMatrix(xgb_predictions, as.factor(test_set_RF_noslice$SHOT_RESULT))
```
