...
#so I can choose to drop some or all or just drop "obvious" errors and recode others or not include pts_type in the model at all
#######
shots %>% filter(SHOT_DIST > 40) %>% nrow(.)
#even at 40 ft this isn't a "realistic" or "common" shot so these shots will be dropped as "outliers"
...
shots <- shots %>% filter(!(SHOT_DIST > 40 | SHOT_DIST == 0))
#
#fix defender names
...
shots <- shots %>% left_join(positions, by = "player_name") %>% left_join(positions, by = c("CLOSEST_DEFENDER" = "player_name")) %>% rename("POS_DEF" = "Pos.y", "POS_SHOOTER" = "Pos.x", "rid_DEF" = "rid.y", "rid_SHOOTER" = "rid.x")
#
...
#now fit a model
fit_glm_shots <- glm(SHOT_NUMERIC ~ GAME_CLOCK + SHOT_CLOCK + DRIBBLES + TOUCH_TIME + SHOT_DIST + PTS_TYPE + CLOSE_DEF_DIST + POS_DEF*POS_SHOOTER, data = train_set_glm_shots, family = "binomial")
